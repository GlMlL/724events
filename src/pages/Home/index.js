...
const Page = () => {
  const { data } = useData(); // Accède aux données du contexte
  const [lastEvent, setLastEvent] = useState(null); // Pour stocker le dernier événement
  ...
  // Cela garantit que l'événement le plus récent est sélectionné et mis à jour dans l'état lastEvent
